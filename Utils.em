...
macro MultiLineComment()
{
    ...
    LnFirst = GetWndSelLnFirst(hwnd)      //Get first line number
    LnLast = GetWndSelLnLast(hwnd)      // get last line number
    hbuf = GetCurrentBuf()
    ...
    while(Ln <= Lnlast) {
        buf = GetBufLine(hbuf, Ln)  //get line Ln
        if(buf == ""){                    // jump over null line
            Ln = Ln + 1
            continue
        }
 
        if(StrMid(buf, 0, 1) == "/") {       //uncomment, in case of single character
            if(StrMid(buf, 1, 2) == "/"){
                PutBufLine(hbuf, Ln, StrMid(buf, 2, Strlen(buf)))
            }
        }
 
        if(StrMid(buf,0,1) != "/"){          // comment
