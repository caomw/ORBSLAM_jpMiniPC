...
{    
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin() , lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        for(vector<MapPoint*>::iterator vit=vpMPs.begin(), vend=vpMPs.end(); vit!=vend; vit++)
        {
            ...
            if(pMP)
                if(!pMP->isBad())
                    if(pMP->mnBALocalForKF!=pKF->mnId)		//mnBALocalForKF, for debug???
                    {
                        lLocalMapPoints.push_back(pMP);
                        pMP->mnBALocalForKF=pKF->mnId;
                    }
	...
	// ////BlockSolverX is dynamic size!! others are BlockSolver_6_3 or _7_3 or _3_2.
	// ////so i can set the current KF as Sim3, others as SE3 ???
    linearSolver = new g2o::LinearSolverCholmod<g2o::BlockSolverX::PoseMatrixType>();	
...
//    vector<g2o::EdgeSE3ProjectXYZ*> vpEdges;
	...
	//                vpEdges.push_back(e);
			...
			else{	// pKFi is current KF, edge is between Sim3 and PointXYZ
				if(!pKFi->isBad())
				{
					Eigen::Matrix<double,2,1> obs;
					cv::KeyPoint kpUn = pKFi->GetKeyPointUn(mit->second);
					...
					g2o::EdgeSim3ProjectXYZ* e = new g2o::EdgeSim3ProjectXYZ();
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
		...
		{
			g2o::VertexSim3Expmap* vSim3 = static_cast<g2o::VertexSim3Expmap*>(optimizer.vertex(pKFi->mnId));
...
//    	{
	        ...
	        vSim3->setEstimate(gScw);	//init value	
	        vSim3->setId(pKFi->mnId);
			// fix keyframe 1 and 2
			bool tmpbFixed = ((pKFi->mnId==0)||(pKFi->mnId==1));
	        vSim3->setFixed(tmpbFixed);
			
			bool tmpbFixed2 = ((pKFi->mnId==0)||(pKFi->mnId==1)||(pKFi->mnId<pKF->mnId-5));
		vSim3->_fix_scale= tmpbFixed2;
...
//            maxKFid=pKFi->mnId;
	...
	//Added by wangjing
    vector<g2o::EdgeSim3ProjectXYZ*> vpEdges;
//    vector<void*> vpEdges;
    vpEdges.reserve(nExpectedSize);
...
//printf("\nhere ok. before optimize\n");

    optimizer.initializeOptimization();
    optimizer.optimize(5);
	
//	printf("\nhere ok. after optimize\n");
...
//		{
    	//
//	        g2o::EdgeSim3ProjectXYZ* e = static_cast<g2o::EdgeSim3ProjectXYZ*>(vpEdges[i]);
	        g2o::EdgeSim3ProjectXYZ* e = vpEdges[i];
	        MapPoint* pMP = vpMapPointEdge[i];
